...
Class Action {
	...
	function save_category(){
		...
		if(!empty($_FILES['img']['tmp_name'])){
			$fname = strtotime(date("Y-m-d H:i"))."_".$_FILES['img']['name'];
			$move = move_uploaded_file($_FILES['img']['tmp_name'], '../img/'.$fname);
			if($move){
				$data .=", img_path = '$fname' ";
			}
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO design_specialty set ".$data);
		}else{
			$save = $this->db->query("UPDATE design_specialty set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function save_design(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO designs set ".$data);
		}else{
			$save = $this->db->query("UPDATE designs set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function delete_category(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM design_specialty where id = ".$id);
		if($delete)
			return 1;
	}
	function delete_cat(){
	...
	function save_designer(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO designers_list set ".$data);
			$did= $this->db->insert_id;
		}else{
			$save = $this->db->query("UPDATE designers_list set ".$data." where id=".$id);
		}
		if($save){
			$data = " username = '$email' ";
			if(!empty($password))
	...
	function save_schedule(){
		...
		foreach($days as $k => $val){
			...
			if(isset($check[$k])){
				if($check[$k]>0)
				$save[] = $this->db->query("UPDATE designer_schedule set ".$data." where id =".$check[$k]);
			else
				$save[] = $this->db->query("INSERT INTO designer_schedule set ".$data);
			}
		}
